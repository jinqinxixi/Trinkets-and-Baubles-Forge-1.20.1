...
public class TitanRingItem extends ModifiableBaubleItem {
    ...
    private void applyFaelisBuff(LivingEntity entity) {
        ...
        if (entity instanceof ServerPlayer serverPlayer) {
            ...
            if (RaceEffectUtil.hasMultipleRaceRings(serverPlayer)) {
                // 如果有多个种族戒指，移除效果
                serverPlayer.removeEffect(ModEffects.TITAN.get());
                return;
            }

            // 检查玩家是否已经有效果
            if (!serverPlayer.hasEffect(ModEffects.TITAN.get())) {
                // 先清除所有种族效果
    ...
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        ...
        if (entity instanceof ServerPlayer serverPlayer) {
            ...
            // 只在装备且没有效果时给予效果
            if (isEquipped(entity) && !serverPlayer.hasEffect(ModEffects.TITAN.get())) {
